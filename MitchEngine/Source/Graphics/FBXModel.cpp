...
void FBXModel::Draw()
{
	for (Moonlight::Mesh* mesh : Meshes)
	{
		mesh->Draw(*ModelShader);
	}
}

void FBXModel::SetShader(Moonlight::Shader* shader)
{
	ModelShader = shader;
}

Moonlight::Mesh* FBXModel::ProcessMesh(FbxMesh* mesh, const FbxScene* scene, int& vertexCounter, FbxNode* child)
{
	std::vector<Moonlight::Vertex> vertices;
	std::vector<unsigned int> indices;
	std::vector<Moonlight::Texture*> textures;
	...
	return new Moonlight::Mesh(vertices, indices, textures);
}
...
void FBXModel::LoadMaterial(FbxNode* node, FbxMesh* mesh, std::vector<Moonlight::Texture*>& materials)
{
...
void FBXModel::LoadMaterialTextures(FbxSurfaceMaterial* material, std::vector<Moonlight::Texture*>& materials)
{
