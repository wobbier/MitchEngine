...
namespace ultralight
{
	...
	void GPUDriverD3D11::CreateTexture(uint32_t texture_id, Ref<Bitmap> bitmap)
	{
		...
		if (bitmap->IsEmpty())
		{
			...
			texture_entry.first = GetEngine().GetRenderer().m_framebuffer->UITexture;
			hr = S_OK;
		}
		else
		{
	...
	void GPUDriverD3D11::LoadShaders()
	{
		...
		Path vsPath = Path("Assets/vs/v2f_c4f_t2f.hlsl");
		Path psPath = Path("Assets/ps/fill_path.hlsl");
		auto& device = GetEngine().GetRenderer().GetDevice();
		Moonlight::ShaderProgram prog = device.CreateShaderProgram(
			device.CompileShader(vsPath, "VS", "vs_4_0"),
			device.CompileShader(psPath, "PS", "ps_4_0"),
			&layout_2f_4ub_2f
		);
		vertex_layout_2f_4ub_2f_ = prog.InputLayout;
		//LoadVertexShader(StringUtils::ToWString(vsPath.FullPath).c_str(), shader_fill_path.first.GetAddressOf(),
  //	  layout_2f_4ub_2f, ARRAYSIZE(layout_2f_4ub_2f), vertex_layout_2f_4ub_2f_.GetAddressOf());
	//LoadPixelShader(StringUtils::ToWString(psPath.FullPath).c_str(), shader_fill_path.second.GetAddressOf());
	...
	//LoadCompiledPixelShader(fill_path_fxc, fill_path_fxc_len, shader_fill_path.second.GetAddressOf());
		shaders_[kShaderType_FillPath] = prog;
		...
		vsPath = Path("Assets/vs/v2f_c4f_t2f_t2f_d28f.hlsl");
		psPath = Path("Assets/ps/fill.hlsl");
		prog = device.CreateShaderProgram(
			device.CompileShader(vsPath, "VS", "vs_4_0"),
			device.CompileShader(psPath, "PS", "ps_4_0"),
			&layout_2f_4ub_2f_2f_28f
		);
		vertex_layout_2f_4ub_2f_2f_28f_ = prog.InputLayout;
		/*
		...
		*/
		...
		shaders_[kShaderType_Fill] = prog;
	}
	...
	void GPUDriverD3D11::BindShader(uint8_t shader)
	{
		...
		{
			Moonlight::ShaderProgram& shader = shaders_[kShaderType_Fill];
