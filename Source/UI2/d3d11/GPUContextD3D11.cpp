...
namespace ultralight {
...
#if defined(_DEBUG)
		ID3D11Debug* debug;
		HRESULT result = GetEngine().GetRenderer().GetDevice().GetD3DDevice()->QueryInterface(__uuidof(ID3D11Debug), reinterpret_cast<void**>(&debug));
	...
	void GPUContextD3D11::Resize(int width, int height) {
		...
		//GetEngine().GetRenderer().GetDevice().GetSwapChain()->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)& back_buffer);
		//back_buffer->Release();
		...
		//HRESULT hr;
		//hr = GetEngine().GetRenderer().GetDevice().GetSwapChain()->ResizeBuffers(0, client_width, client_height, DXGI_FORMAT_UNKNOWN, 0);
		//if (FAILED(hr)) {
		//	MessageBoxW(nullptr,
	...
	ID3D11Device* GPUContextD3D11::device() { assert(GetEngine().GetRenderer().GetDevice().GetD3DDevice()); return GetEngine().GetRenderer().GetDevice().GetD3DDevice(); }
	ID3D11DeviceContext* GPUContextD3D11::immediate_context()
	{
		ID3D11DeviceContext* ptr = GetEngine().GetRenderer().GetDevice().GetD3DDeviceContext();
		assert(ptr);
		return ptr;
	}
	...
	bool GPUContextD3D11::Initialize(HWND hWnd, int screen_width, int screen_height, double screen_scale, bool fullscreen, bool enable_vsync, bool sRGB, int samples) {
		...
		//  // Unable to create device  & swap chain
		   // YIKES("Failed to make SWAP chain");
		//  return false;
		//}
		...
		//if (FAILED(hr))
		//{
		   // YIKES("Unable create back buffer view");
		...
		m_renderer->GetDevice().GetD3DDeviceContext()->OMSetBlendState(blend_state_.Get(), NULL, 0xffffffff);
		...
		m_renderer->GetDevice().GetD3DDeviceContext()->RSSetState(rasterizer_state_.Get());
		...
		m_renderer->GetDevice().GetD3DDeviceContext()->ClearRenderTargetView(render_target_view(), color);
