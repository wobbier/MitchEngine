...
namespace ImGui
{
	...
	void DrawCurve(ImVec2* results, ImRect& bb, ImDrawList* DrawList)
	{
	...
	bool DrawCurve(float P[5], float width, float height)
	{
		...
		ImVec2 Canvas(dim, dim2);

		ImRect bb(GetCurrentWindow()->DC.CursorPos, GetCurrentWindow()->DC.CursorPos + Canvas);

		ImVec2 Q[4] = { { 0, 0 }, { P[0], P[1] }, { P[2], P[3] }, { 1, 1 } };
		ImVec2 results[SMOOTHNESS + 1];
		bezier_table<SMOOTHNESS>(Q, results);
		DrawCurve(results, bb, GetWindowDrawList());
		return ImGui::IsMouseClicked(ImGuiMouseButton_Left) && ImGui::IsMouseHoveringRect(bb.Min, bb.Max);
	}
	...
	int Bezier(const char* label, float P[5]) {
		...
		enum { AREA_HEIGHT = 0 }; // area width in pixels. 0 for adaptive size (will use max avail width)
